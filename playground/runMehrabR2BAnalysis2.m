...
ridge_h_width_f = round(ridge_h_width/frame_time); %in frames
...
if non_ov_trials == 0
    kernels = mean(dff_data_mat(:, cell_list, pk_behav_trial:no_trials), 3, 'omitnan');

elseif non_ov_trials == 1
    k_tr_list = pk_behav_trial:2:no_trials;

    kernels = mean(dff_data_mat(:, cell_list, k_tr_list), 3, 'omitnan'); %2D matrix: (values, cells)
...
kernels_e = mean(dff_data_mat(:, cell_list, 1:5), 3,'omitnan');  %for early trials only; 2D matrix: (values, cells)
...
for cell_noi = 1:length(cell_list)
    ...
    if isnan(sum(trace, 'omitnan'))
        %error('nansum(trace) is NaN for cell: %i', cell_noi)
        ...
        ridge_int = (sum(trace( (pk_f - ridge_h_width_f):(pk_f + ridge_h_width_f) ), 'omitnan'));
        ...
        if isempty(trace_x) || abs(bk_int) == 0 %|| bk_int == 0
            ...
            for iter_r = 1:r_iters
                ...
                s_trace_av = mean(s_trace_av_mat, 2, 'omitnan');
                ...
                ridge_int = sum(s_trace_av( (pk_f - ridge_h_width_f):(pk_f + ridge_h_width_f) ), 'omitnan');
                ridge_int = ridge_int./(2.*ridge_h_width_f + 1);                                    %normalising by number of points considered
            ...
            rrb_ratio_vec_final(1, cell_noi) = mean(rrb_ratio_vec, 2, 'omitnan');
