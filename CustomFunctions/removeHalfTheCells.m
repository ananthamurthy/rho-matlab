...
for dataseti = 1:length(sdo_batch)
    selectedCells4Removal = (sort(randi(nCells, 1, nCells2Remove)));
    
    myDataset = sdo_batch(dataseti).syntheticDATA;
    myDataset_2D = sdo_batch(dataseti).syntheticDATA_2D;

    myDataset(selectedCells4Removal, :, :) = [];
    myDataset_2D(selectedCells4Removal, :) = [];

    sdo_batch(dataseti).syntheticDATA = myDataset;
    sdo_batch(dataseti).syntheticDATA_2D = myDataset_2D;
